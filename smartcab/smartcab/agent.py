...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            self.t+=1.0
            #epsilon decay function options:
            #self.epsilon = self.epsilon - 0.05
            #self.epsilon=math.pow(self.alpha,self.t)
            #self.epsilon=1/(self.t**2)
            self.epsilon=math.fabs(math.exp(-self.alpha*self.t))
    ...
    def get_maxQ(self, state):
        ...
        #build a lambda function to get the Q actions in certain state
        maxaction=max(self.Q[state],key=lambda x:self.Q[state][x])
        #from state and max action, get maxQ
    ...
    def createQ(self, state):
        ...
        if self.learning:
            #set default value by get function
    ...
    def choose_action(self, state):
        ...
        # while learning process simulated annealing to find global maximum
    ...
    def learn(self, state, action, reward):
        ...
        # Input Q-learning equation
        action_t=self.Q[state][action]
        if self.learning:
            self.Q[state][action]=(1-self.alpha)*action_t+self.alpha*(reward)
...
def run():
    ...
    sim.run(n_test=177,tolerance=0.001)
